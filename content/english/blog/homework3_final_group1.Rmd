---
description: The Relationship between Physical Activity and Weight  # the title that will show up once someone gets to this page
draft: false
image: images/slider/Hockey_Team.jpeg # save picture in \static\img\blogs. Acceptable formats= jpg, jpeg, or png . Your iPhone pics wont work

keywords: ""
slug: phys_act # slug is the shorthand URL address... no spaces plz
...
Null hypothesis: The difference between the mean ratings of the two director's movies is zero.

Alternative hypothesis: The difference between the mean ratings of the two director's movies is not zero.

t-stat: 2.7144 (using direct t_test)

p-value: 0.01078 (using t_test), 0.008 (using infer package)

Conclusion: The t-stat is greater than 2 ( the p-value is smaller than 0.05), so we should reject the null hypothesis, and conclude that the mean ratings of Steven Spielberg and Tim Burton's movies are not the same.
...
# Summary stats for both directors
...
# Create a plot for confidence intervals of ratings for movies by the two directors
ggplot(filteredmovies,aes(x=mean_rating,y=director,color=director))+
    geom_point(size=2.5)+
   geom_text(aes(label=round(mean_rating,2),x=mean_rating,size=8),vjust=-1,color="black")+
#plot the errorbar
geom_errorbarh(aes(y=director,xmin = rating_low, xmax =        rating_high,color=director),height=0.05,size=1)+
geom_text(aes(label=round(rating_low,2),x=rating_low),vjust=-1,color="black")+
geom_text(aes(label=round(rating_high,2),x=rating_high),vjust=-1,color="black")+
#plot the grey rectangle
geom_rect(aes(xmin=filteredmovies$rating_low[[1]],xmax=filteredmovies$rating_high[[2]],ymin=-Inf,ymax=Inf),color = NA,alpha=0.3)+
  theme_bw()+
